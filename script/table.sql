...
(
    ...
    post_id       bigint   not null comment '评论所属文章(考虑性能，不加约束)',
    user_id       bigint   not null comment '评论者id(考虑性能，不加约束)',
    reply_user_id bigint   not null comment '回复评论(考虑性能，不加约束)',
    parent_id     bigint   not null comment '回复的父评论id',
...
(
    ...
    user_id    bigint       not null comment '作者id（考虑性能，不加外键）'
    )
    comment '草稿表';
...
(
    ...
    gender          tinyint      not null comment '用户性别：0女，1男，2其他',
    college         varchar(30)  not null comment '用户所在学院',
    major           varchar(30)  not null comment '用户专业',
    grade           tinyint      not null comment '用户年级：0大一，1大二，2大三，3大四，4研究生',
...
(
    ...
    plate_id       bigint       null comment '板块id',
...
(
    ...
    categorie_id bigint          not null comment '隶属哪个归档',
...
(
    id         bigint   null comment '自然主键',
    created_at datetime not null comment '创建时间',
    deleted_at datetime null comment '删除时间（软删除）',
    updated_at datetime null comment '更新时间',
    comment_id bigint   not null comment '评论id（考虑性能，不加外键）',
    user_id    bigint   not null comment '点赞人（考虑性能，不加外键）'
)
    comment '评论-点赞表';
...
(
    ...
    categorie_id bigint      null comment '该标签隶属的归档',
    constraint tag_pk2
    unique (name),
...
(
    ...
    deleted_at datetime null comment '删除时间（软删除）',
    updated_at datetime null comment '更新时间',
    tag_id     bigint   null comment '文章所属标签',
    post_id    bigint   null comment '博文id',
...
(
    ...
    topic_id    bigint          null comment '所属话题',
    user_id     bigint          not null comment '随笔作者（考虑性能，不加外键）',
...
(
    ...
    deleted_at datetime null comment '删除时间（软删除）',
    updated_at datetime null comment '更新时间',
    topic_id   bigint   not null comment '话题id（考虑性能，不加外键）',
    user_id    bigint   not null comment '点赞人（考虑性能，不加外键）'
)
    comment '话题-点赞表';
...
(
    ...
    created_at  datetime not null comment '创建时间',
    deleted_at  datetime null comment '删除时间（软删除）',
    user_id     bigint   not null comment 'subject',
    followed_id bigint   not null comment 'object',
