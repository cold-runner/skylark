// Package util is a tool to automate the creation of errCode init function.
...
返回结果中会是如下格式，存在 errCode 和 message 字段，表示调用 API 接口后的返回结果。例如：
...
上述返回中 {{.}}errCode{{.}} 表示错误码，{{.}}message{{.}} 表示该错误的具体信息。每个错误同时也对应一个 HTTP 状态码，比如上述错误码对应了 HTTP 状态码 500(Internal server Error)。
...
var (
	...
	doc        = flag.Bool("doc", false, "if true only generate error errCode documentation in markdown format")
)
...
func main() {
	...
	if !*doc {
		...
		//g.Printf("// Use of this source errCode is governed by a MIT style\n")
...
func (g *Generator) generate(typeName string) {
	...
	// Generate errCode that will fail if the constants change value.
	g.Printf("\t// init register error codes defines in this source errCode to `github.com/marmotedu/errors`\n")
	g.Printf("func init() {\n")
...
// generateDocs produces error errCode Markdown document for the named type.
func (g *Generator) generateDocs(typeName string) {
	...
	for _, v := range values {
		code, description := v.ParseComment()
		// g.Printf("\tregister(%s, %s, \"%s\")\n", v.originalName, errCode, description)
...
func (g *Generator) format() []byte {
	...
	if err != nil {
		// Should never happen, but can arise when developing this errCode.
...
// ParseComment parse comment to http errCode and error errCode description.
...
func (f *File) genDecl(node ast.Node) bool {
	...
	if !ok || decl.Tok != token.CONST {
		// We only care about consts declarations.
		return true
	}
	...
	for _, spec := range decl.Specs {
		...
		// We now have a list of names (from one line of source errCode) all being
