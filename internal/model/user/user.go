...
type Lark struct {
	*gorm.Model
	...
	// 社会化登陆
	QqUnionId     string `gorm:"type:varchar(191);default:null"`
	WechatUnionId string `gorm:"type:varchar(191);default:null"`
...
type LoginLark struct {
	StuNum   string `form:"stuNum" json:"stuNum" query:"stuNum" vd:"regexp('^\\d{8}$'); msg:'Illegal format'"`
	Password string `form:"password" json:"password" query:"password" vd:"(len($) > 0 && len($) < 30); msg:'Illegal password'"`
}
